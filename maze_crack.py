...
walls = re.findall('<line\s+x1="(\d+)"\s*x2="(\d+)"\s*y1="(\d+)"\s*y2="(\d+)"\s*stroke="black"\s*stroke-width="3"', maze_raw_data)
...
def separated_by_wall(connection):
	...
	if y1 == y2:
		minx = min(x1, x2)
		maxx = max(x1, x2)
		separators = [1 for ((wx1, wy1), (wx2, wy2)) in walls if wx1 == wx2 and wx1 > minx and wx1 < maxx and y1 > min(wy1, wy2) and y1 < max(wy1, wy2)]
	else:
		miny = min(y1, y2)
		maxy = max(y1, y2)
		separators = [1 for ((wx1, wy1), (wx2, wy2)) in walls if wy1 == wy2 and wy1 > miny and wy1 < maxy and x1 > min(wx1, wx2) and y1 < max(wx1, wx2)]
...
def getSuccessors(connections, node):
	return [target for (origin, target) in connections if origin == node]

def traverse(connections, start, until, seen = []):
	...
	seen = seen + [start]
	for successor in getSuccessors(connections, start):
		if not successor in seen and traverse(connections, successor, until, seen):
				return True
	return False


print(traverse(validConnections, , (4,3)))
